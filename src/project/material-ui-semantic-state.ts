...
export function extractMaterialUiSemanticBehaviors(sourceFile: ts.SourceFile): RenderStateBehaviorContract[] {
  const behaviors = [
    ...
    ...extractSemanticVisibilityRule(sourceFile),
    ...extractSemanticAccessibilityRule(sourceFile),
  ];
  const seen = new Set<string>();
  return behaviors.filter((behavior) => {
    const key = [behavior.componentName, behavior.kind, behavior.condition.prop, String(behavior.condition.value), JSON.stringify(behavior.expectation)].join('|');
